...
from .net import complexity_conv2d,complexity_batchnorm2d,complexity_maxpool2d
...
class VanillaBlock(nn.Module):
    ...
    def complexity(cx, w_in, w_out, stride, bm=None, gw=None, se_r=None):
        err_str = "Vanilla block does not support bm, gw, and se_r options"
        assert bm is None and gw is None and se_r is None, err_str
        cx = complexity_conv2d(cx, w_in, w_out, 3, stride, 1)
        cx = complexity_batchnorm2d(cx, w_out)
        cx = complexity_conv2d(cx, w_out, w_out, 3, 1, 1)
        cx = complexity_batchnorm2d(cx, w_out)
        return cx
...
class SE(nn.Module):
    ...
    def complexity(cx, w_in, w_se):
        h, w = cx["h"], cx["w"]
        cx["h"], cx["w"] = 1, 1
        cx = complexity_conv2d(cx, w_in, w_se, 1, 1, 0, bias=True)
        cx = complexity_conv2d(cx, w_se, w_in, 1, 1, 0, bias=True)
        cx["h"], cx["w"] = h, w
        return cx
...
class BottleneckTransform(nn.Module):
    ...
    def complexity(cx, w_in, w_out, stride, bm, gw, se_r):
        w_b = int(round(w_out * bm))
        g = w_b // gw
        cx = complexity_conv2d(cx, w_in, w_b, 1, 1, 0)
        cx = complexity_batchnorm2d(cx, w_b)
        cx = complexity_conv2d(cx, w_b, w_b, 3, stride, 1, g)
        cx = complexity_batchnorm2d(cx, w_b)
        if se_r:
            w_se = int(round(w_in * se_r))
            cx = SE.complexity(cx, w_b, w_se)
        cx = complexity_conv2d(cx, w_b, w_out, 1, 1, 0)
        cx = complexity_batchnorm2d(cx, w_out)
        return cx
...
class ResStemCifar(nn.Module):
    ...
    def complexity(cx, w_in, w_out):
        cx = complexity_conv2d(cx, w_in, w_out, 3, 1, 1)
        cx = complexity_batchnorm2d(cx, w_out)
        return cx
...
class ResStemIN(nn.Module):
    ...
    @staticmethod
    def complexity(cx, w_in, w_out):
        cx = complexity_conv2d(cx, w_in, w_out, 7, 2, 3)
        cx = complexity_batchnorm2d(cx, w_out)
        cx = complexity_maxpool2d(cx, 3, 2, 1)
        return cx
...
class SimpleStemIN(nn.Module):
    ...
    @staticmethod
    def complexity(cx, w_in, w_out):
        cx = complexity_conv2d(cx, w_in, w_out, 3, 2, 1)
        cx = complexity_batchnorm2d(cx, w_out)
        return cx
